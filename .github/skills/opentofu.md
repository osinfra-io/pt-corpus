...
- Regional workspace: `{region}-{env}` (e.g., `us-east1-production`)
...
### pt-corpus (Team Infrastructure Layer)

**Purpose:** Manages team-specific Google Cloud projects and infrastructure.

**Unique Characteristics:**

- **Multi-environment pattern:** Sandbox → Non-Production → Production progression
- **Two-level structure:** Root level + `regional/` subdirectory
- **Environment-based tfvars:** Uses `environments/` directory (e.g., `sandbox.tfvars`, `production.tfvars`)
- **Helpers module integration:** Consumes team data and organizational structure from pt-logos
- **Shared VPC networking:** Regional deployments manage VPC, subnets, and network resources

**Workflow:** PR triggers sandbox → Merge triggers non-production → Success triggers production.
